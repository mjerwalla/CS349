/* Enhancements: 
   Added different colours for the frog and each block type :
...
public:
    virtual void paint(XInfo& xinfo) {
        ...
        char colour[8]; 
        if (this->w == 20) {
            strcpy(colour,"#0B16F2");
        } else if (this->w == 50) {
            strcpy(colour,"#900C3F");
        } else {
            strcpy(colour,"#FF0202");
        }
        colourMap = DefaultColormap(xinfo.display, 0);
        XParseColor(xinfo.display, colourMap, colour, &colourChoice);
        XAllocColor(xinfo.display, colourMap, &colourChoice);
        XSetForeground(xinfo.display, gc, colourChoice.pixel);
...
public:
    virtual void paint(XInfo& xinfo) {        
        ...
        if (playerNum == 1){
            if (levelNum % 2 == 0) {
                strcpy(colour, "#02FAFA");
            } else {
                strcpy(colour, "#02FCB8");
            }

        } else {
            if (levelNum %2 == 0){
                strcpy(colour, "#FC02C3");
            } else {
                strcpy(colour, "#6102FC");
            }
        }
        colourMap = DefaultColormap(xinfo.display, 0);
        XParseColor(xinfo.display, colourMap, colour, &colourChoice);
    ...
    Frog(int x= 425, int y=200, int playerNum = 1): x(x), y(y), playerNum(playerNum){}
...
void eventloop(XInfo& xinfo) {
    ...
    // Initializing frog     
    Frog* frog = new Frog(475);
    dList.push_back(frog);
    Frog* frog1 = new Frog(375, 200, 2);
    if (player > 1) {
    ...
    while ( true ) {
        ...
        if (XPending(xinfo.display) > 0) {
            ...
            case KeyPress: // any keypress
                ...
                if ( i == 1 && text[0] == 'n' ) {
                    ...
                    if (frog->getY() < 50 ) {
                        if ((player > 1 && frog1->getY() < 50) || player == 1){
                            ++levelNum; 
        ...
        if (player > 1) {
            if (collision(collisionList, frog1)) {
                levelNum = 1; 
                frog->setX(475);
                frog->setY(200);
                frog1->setX(375);
